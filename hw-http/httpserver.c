...
void writef(int fd, void *buf, ssize_t length) {
  ssize_t wsize;
  wsize = write(fd, buf, length);
  while (wsize < length) {
    wsize += write(fd, buf + wsize, length - wsize);
  } 
}

void *hostsend(void *socketfd) {
  ssize_t buffersize;
  char buff[800];
  int *fd = socketfd;
  int hostfd = fd[0];
  int proxyfd = fd[1];
  while (proxy_status == 0) {  
    buffersize = recv(hostfd, buff, 800, 0);
    if (buffersize > 0) {
      writef(proxyfd, buff, buffersize);
    } else {
      proxy_status = 1;
    }
  }
...
void *proxysend(void *socketfd) {
  ...
  int proxyfd = fd[1];
  while (proxy_status == 0) {  
    buffersize = recv(proxyfd, buff, 800, 0);
    if (buffersize > 0) {
      writef(hostfd, buff, buffersize);
    } else {
      proxy_status = 1;
    }
  }
...
void *thread_server_helper (void *socketfd) {
  void **fd = socketfd;
  int *thead_num = fd[0];
  void (**thread_helper)(int) = fd[1];
  (**thread_helper)(*thead_num);
  pthread_exit(0);
}
...
void serve_file(int fd, char* path) {
  ...
  stat(path, &pstat);
  flength = (long long) pstat.st_size;
  snprintf(content_length, 50, "%lld", flength);
  ...
  rlength = read(pfd, buf, 100);
  while (rlength != 0 && rlength != -1) {
    writef(fd, buf, rlength);
    rlength = read(pfd, buf, 100);
  }
  close(pfd);
  /* PART 2 END */
...
void serve_directory(int fd, char* path) {
  ...
  writef(fd, header, strlen(header));
  while ((dirstream = readdir(dirpath))) {
    http_format_href(buf, path, dirstream->d_name);
    writef(fd, buf, strlen(buf));
  }
  writef(fd, footer, strlen(footer));
  closedir(dirpath);
  /* PART 3 END */
}
...
void handle_files_request(int fd) {
  ...
  if (stat(path, &pstat) == 0) { 
    if (S_ISREG(pstat.st_mode)) {
      ...
      dirpath = opendir(path);
      while ((dirstream = readdir(dirpath))) {
        if (strcmp(dirstream->d_name, "index.html") == 0) {
          index = 1;
          buf = malloc(strlen(path) + 12);
...
void handle_proxy_request(int fd) {
  ...
  */
  ...
  pthread_t proxy;
  int *socketfd;
  socketfd = (int*) malloc(sizeof(int) * 2);
  socketfd[0] = fd;
  socketfd[1] = target_fd;
  proxy_status = 0;

  if (pthread_create(&host, NULL, &hostsend, (void *) socketfd) != 0) {
    exit(errno);
  }
  if (pthread_create(&proxy, NULL, &proxysend, (void *) socketfd) != 0) {
    exit(errno);
  }
...
void* handle_clients(void* void_request_handler) {
  ...
  /* PART 7 BEGIN */
  while (1) {
    int clientfd = wq_pop(&work_queue);
...
void init_thread_pool(int num_threads, void (*request_handler)(int)) {
  ...
  init_threads = (pthread_t *) malloc(sizeof(pthread_t) * num_threads);
  for (size_t i = 0; i < num_threads; i++) {
    pthread_create(init_threads + i, NULL, &handle_clients, request_handler);
  }
  /* PART 7 END */
}
#endif
...
void serve_forever(int* socket_number, void (*request_handler)(int)) {
  ...
  *socket_number = socket(PF_INET, SOCK_STREAM, 0);
  ...
  if (bind(*socket_number, (struct sockaddr *) &server_address, sizeof(server_address)) == -1) {
    perror("Failed to bind to an address and port");
    exit(errno);
  }

  if (listen(*socket_number, 1024) == -1) {
    perror("Failed to listen with scocket");
    exit(errno);
  }
  ...
  while (1) {
    ...
    arg[0] = &client_socket_number;
    arg[1] = &request_handler;
    pthread_create(&cthread, NULL, &thread_server_helper, (void *)arg);
