...
static void* syscall_sbrk(intptr_t increment) {
  struct thread *t = thread_current();
	void *addr;
  void *growth = t->heapEnd;

	if (increment < 0) {
		if (growth + increment < t->heapStart) {
			increment = growth- t->heapStart;
    }
		size_t ramp = (pg_round_up(growth) - pg_round_up(growth + increment)) / PGSIZE;
		growth = pg_round_up(growth + increment);
		for (size_t pt = 0; pt < ramp; pt++) {
			addr = pagedir_get_page(t->pagedir, growth + PGSIZE * pt);
			pagedir_clear_page(t->pagedir, growth + PGSIZE * pt);
			palloc_free_page(addr);
		}
	} else if (increment > 0) {
	  size_t ramp = (pg_round_up(growth + increment) - pg_round_up(growth)) / PGSIZE;
		if (!(addr = palloc_get_multiple(PAL_ZERO | PAL_USER, ramp)) && ramp > 0) {
			return (void *)-1;
    }
		growth = pg_round_up(growth);
		for (size_t pt = 0; pt < ramp; pt++) {
			if (!pagedir_set_page(t->pagedir, growth + PGSIZE * pt, addr + PGSIZE * pt, true)) {
				return (void *)-1;
      }
    }
	}
