...
static void page_fault(struct intr_frame* f) {
   ...
   * be grown.
   */
   if (user && ((f->esp - 32) > fault_addr)) {
      syscall_exit(-1);
   } else {
      size_t uaddr = (size_t)((void *)t->stackStart - pg_round_down(fault_addr)) / PGSIZE;
		void *addr = palloc_get_multiple(PAL_ZERO | PAL_USER, uaddr);
      if (addr == NULL) {
         syscall_exit(-1);
      }
		t->stackStart = pg_round_down(fault_addr);
		for (size_t pt= 0; pt < uaddr; pt++) {
			pagedir_set_page(t->pagedir, t->stackStart + PGSIZE * pt, addr + PGSIZE * pt, true);
		}
		return;
   }
